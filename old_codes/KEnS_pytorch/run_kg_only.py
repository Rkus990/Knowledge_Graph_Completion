...
def train_align_batch(align_model,align_data,optimizer):
    ...
    # Adjust learning rate for alignment module
    # for param_group in optimizer.param_groups:  # 在每次更新参数前迭代更改学习率
    #     param_group["lr"] = param.align_lr
    #
    # print(optimizer.param_groups)
    #
    # # Adjust Learnable parameters with only the two related KGs
    # for i in range(len(all_kgs)):
    #     #Judge whether it is the two kgs
    #     each_kg = all_kgs[i]
    #     if not each_kg.lang in (align_model.lang,align_model.other_lang):
    #         for name,value in each_kg.named_parameters():
...
def main(args):
    ...
    model_dir = join('./trained_model_kg_only', f'kens-{param.knowledge}-{param.dim}', target_lang)  # output
